...
To resolve this issue in a non-interactive environment, use the `--yes` parameter to avoid any confirmation prompts, which ensures the command completes without requiring user input.
...
az aks update --resource-group $AKS_RG_NAME --name $AKS_CLUSTER_NAME --yes
```
