...
To run a command with a file attached, first create a sample Kubernetes manifest file named `deployment.yaml`. The following example creates a simple nginx deployment manifest.
...
Now you can apply it to your cluster using `az aks command invoke` with the `--file` argument:
...
You can attach all files in your current directory using the `--file .` argument. For this example, also create a simple `configmap.yaml`:
...
This example applies both `deployment.yaml` and `configmap.yaml` in the `default` namespace:
