...
#### Q. What is the limit for discovering and assessing servers in a single Azure Migrate project? 

A. You can discover and assess up to 50,000 servers in a VMware environment in a single project. A project can include physical servers and servers from a Hyper-V environment, up to the assessment limits.


#### Q. How many vCenter Servers can the Azure Migrate appliance support? 

A. The Azure Migrate appliance supports adding multiple vCenter Servers. You can add up to 10 vCenter Servers per appliance.


#### Q. What version of vCenter Server is supported for server discovery and assessment? 

A. Servers that you want to discover and assess must be managed by vCenter Server version 8.0, 7.0, 6.7, 6.5, 6.0, or 5.5. Discovering servers by providing ESXi host details in the appliance currently isn't supported.


#### Q. What permissions are required for the vCenter Server account used by Azure Migrate? 

A. The Azure Migrate: Discovery and assessment tool requires a vCenter Server read-only account. If you want to use the tool for software inventory, agentless dependency analysis, web apps, and SQL discovery, the account must have privileges for guest operations on VMware VMs.


#### Q. What operating systems are supported for assessment? 

A. All Windows and Linux operating systems can be assessed for migration.


#### Q. What are the port access requirements for the Azure Migrate Appliance? 

A. Inbound connections on TCP port 3389 are required to allow remote desktop connections to the appliance. Inbound connections on port 44368 are required to remotely access the appliance management app using the URL `https://<appliance-ip-or-name>:44368`. Outbound connections on port 443 (HTTPS) are required to send discovery and performance metadata to Azure Migrate.


#### Q. What are the port access requirements for vCenter Server? 

A. Inbound connections on TCP port 443 are required to allow the appliance to collect configuration and performance metadata for assessments. The appliance connects to vCenter on port 443 by default. If vCenter Server listens on a different port, you can modify the port when you set up discovery.


#### Q. What are the port access requirements for ESXi hosts? 

A. For discovery of software inventory or agentless dependency analysis, the appliance connects to ESXi hosts on TCP port 443 to discover software inventory and dependencies on the servers.


#### Q. What are the software requirements for performing software inventory? 

A. For software inventory, VMware Tools must be installed and running on your servers. The VMware Tools version must be version 10.2.1 or later. Windows servers must have PowerShell version 2.0 or later installed. WMI must be enabled and available on Windows servers to gather the details of the roles and features installed on the servers.


#### Q. What are the server requirements for performing software inventory? 

A. You can add multiple domain and non-domain (Windows/Linux) credentials in the appliance configuration manager for software inventory. You must have a guest user account for Windows servers and a standard user account (non-sudo access) for all Linux servers.


#### Q. What are the requirements for discovering SQL Server instances and databases? 

A. To discover SQL Server instances and databases, the Windows or SQL Server account must be a member of the sysadmin server role or have the necessary permissions for each SQL Server instance. The supported SQL Server versions are SQL Server 2008 and later, and the supported SQL Server editions are Enterprise, Standard, Developer, and Express. Discovery of SQL Server Reporting Services (SSRS), SQL Server Integration Services (SSIS), and SQL Server Analysis Services (SSAS) isn't supported.


#### Q. What are the requirements for web apps discovery? 

A. If a server has a web server installed, Azure Migrate can discover web apps on the server. The user can add both domain and non-domain credentials on the appliance. Ensure that the account used has local admin privileges on source servers.


#### Q. What are the requirements for agentless dependency analysis? 

A. Dependency analysis requires VMware Tools (10.2.1 and later) to be installed and running on servers you want to analyze. Servers must have PowerShell version 2.0 or later installed. WMI should be enabled and available on Windows servers. Additionally, a user account (local or domain) with administrator permissions on Windows servers or a sudo user account with permissions to execute ls and netstat commands on Linux servers is required.


#### Q. What are the requirements for agent-based dependency analysis? 

A. To set up agent-based dependency analysis, you need to install the Microsoft Monitoring Agent (MMA) and the Dependency agent on each server you want to analyze. Additionally, you need a Log Analytics workspace associated with the project and internet connectivity for the servers.


#### Q. What are the requirements for agent-based dependency analysis in Azure Government? 

A. Agent-based dependency analysis is not supported in Azure Government.
