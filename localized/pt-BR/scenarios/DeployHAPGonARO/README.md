...
Esse trecho de código executa as seguintes etapas:
...
Nesta seção, você implantará um cluster do Red Hat OpenShift do Azure (ARO). A variável ARO_CLUSTER_NAME conterá o nome do seu cluster do ARO. O comando az aro create implantará o cluster ARO com o nome especificado, o grupo de recursos, a rede virtual, as sub-redes e o segredo pull do RedHat OpenShift que você baixou e salvou anteriormente no Cofre da Chave. Esse processo poderá levar cerca de 30 minutos para ser concluído.
...
Implante uma instância Keycloak em um cluster OpenShift. Ele usa o `oc apply` comando para aplicar um arquivo de configuração YAML que define o recurso Keycloak.
A configuração do YAML especifica várias definições para a instância Keycloak, incluindo o banco de dados, o nome do host, as configurações HTTP, a entrada, o número de instâncias e as configurações de transação.
Para implantar o Keycloak, execute esse bloco de código em um ambiente de shell com as permissões necessárias e acesso ao cluster OpenShift.
