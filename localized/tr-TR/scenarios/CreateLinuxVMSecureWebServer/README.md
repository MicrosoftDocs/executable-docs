---
title: HTTPS ile Güvenliği Sağlanan bir NGINX Web Sunucusu Oluşturma
description: Bu öğreticide HTTPS aracılığıyla güvenliği sağlanan bir NGINX Web Sunucusu oluşturma gösterilmektedir.
author: mbifeld
ms.author: mbifeld
ms.topic: article
ms.date: 11/28/2023
...
# HTTPS ile Güvenliği Sağlanan bir NGINX Web Sunucusu Oluşturma
...
> * Azure Ağı'nın kurulumunu ve güvenliğini sağlama
...
CLI'yi yerel olarak yükleyip kullanmayı seçerseniz, bu öğretici için Azure CLI 2.0.30 veya sonraki bir sürümünü kullanmanız gerekir. Sürümü bulmak için `az --version` komutunu çalıştırın. Yüklemeniz veya yükseltmeniz gerekirse, bkz. [Azure CLI yükleme]( https://learn.microsoft.com//cli/azure/install-azure-cli ).

## Değişken Bildirimi

Bu öğreticiyi yürütmek için ihtiyacınız olan tüm ortam değişkenlerinin listesi:
...
Güvenli bir Linux VM oluşturmadan önce az group create ile bir kaynak grubu oluşturun. Aşağıdaki örnek, REGION* değişken içeriği tarafından belirtilen konumda MY_RESOURCE_GROUP_NAME* değişkeninin *içeriğine *eşit bir kaynak grubu oluşturur:
...
$MY_RESOURCE_GROUP_NAME kaynak grubunda $MY_SN_NAME adlı *bir alt ağ ile $MY_VNET_NAME *** adlı *bir sanal ağ oluşturmak için az network vnet create komutunu kullanın.*
...
$MY_RESOURCE_GROUP_NAME* içinde $MY_PUBLIC_IP_NAME ** adlı *standart bir alanlar arası yedekli genel IPv4 adresi oluşturmak için az network public-ip create komutunu kullanın.
...
SSH ve Web trafiğine izin vermek için 22 (SSH), 80 (HTTP) ve 443 (HTTPS) bağlantı noktalarını açın
...
Ve son olarak Ağ Arabirimi Kartı'nı (NIC) oluşturun:
...
## Sertifika oluşturma ve Azure Key Vault'ta depolama

Azure Key Vault, sertifikalar veya parolalar gibi şifreleme anahtarları ile gizli dizilerin güvenliğini sağlar. Key Vault, sertifika yönetimi işlemini kolaylaştırır ve bu sertifikalara erişen anahtarları denetiminizde tutmanıza olanak sağlar. Key Vault içinde otomatik olarak imzalanan bir sertifika oluşturabilir veya sahip olduğunuz güvenli bir sertifikayı karşıya yükleyebilirsiniz. Bu öğretici için Key Vault'ta otomatik olarak imzalanan sertifikalar oluşturacak ve ardından bu sertifikaları çalışan bir VM'ye ekleyeceğiz. Bu işlem, dağıtım sırasında web sunucusuna en güncel sertifikaların yüklenip yüklenmediğini kontrol eder.

Aşağıdaki örnek, 7 günlük bekletme ilkesine sahip $REGION seçilen bölgede *$MY*_KEY_VAULT* adlı *bir Azure Key Vault oluşturur. Bu, gizli dizi, anahtar, sertifika veya anahtar kasası silindikten sonra 7 ile 90 takvim günü arasında yapılandırılabilir bir süre boyunca kurtarılabilir kalacağı anlamına gelir.
...
## Azure Key Vault'ta sertifika ve depo oluşturma

Şimdi az keyvault certificate create ile varsayılan sertifika ilkesini kullanan otomatik olarak imzalanan bir sertifika oluşturalım:
...
Son olarak, VM oluşturma işlemi sırasında kullanılabilmesi için sertifikayı hazırlamamız gerekir. Bunu yapmak için az keyvault secret list-versions ile sertifikanın kimliğini edinmemiz ve sertifikayı az vm secret biçiminde dönüştürmemiz gerekir. Aşağıdaki örnekte, sonraki adımlarda kullanım kolaylığı sağlamak için bu komutların çıkışı değişkenlere atanmaktadır:
...
Şimdi az vm create ile bir VM oluşturun. cloud-init-nginx.txt* adlı *cloud-init yapılandırma dosyasını geçirmek için --custom-data parametresini kullanın.
...
VM oluşturduğunuzda, sertifika ve anahtarlar korunan /var/lib/waagent/ dizininde depolanır. Bu örnekte, NGINX web sunucusunu yüklüyor ve yapılandırıyoruz.
...
Aşağıdaki örnek myVMName$UNIQUE_POSTFIX* adlı *bir VM oluşturur:
...
## Vm $vm_name için AKV uzantısı dağıtarak AKV $cert $akv_name..."
...
Aşağıdaki örnek bir VM dağıtır ve ardından Bir Linux VM için Azure AD oturum açma özelliğini etkinleştirmek üzere uzantıyı yükler. VM uzantıları, Azure sanal makinelerinde dağıtım sonrası yapılandırma ve otomasyon görevleri sağlayan küçük uygulamalardır.
...
## Güvenli web sitenize göz atın

Uygulama URL'sini ziyaret ederek uygulamanın çalıştığını doğrulayın:
