...
# 수신 컨트롤러가 배포될 때까지 대기합니다. 배포될 때까지 계속 확인합니다.
true이면 </&입니다. do aks_cluster_ip=$(kubectl get ingress -o=jsonpath='{.status.loadBalancer.ingress[0].ip}') if [[ -n "$aks_cluster_ip" ]]; 그런 다음 에코 "AKS 수신 IP 주소는 다음과 같습니다: $aks_cluster_ip" break else echo "AKS 수신 IP 주소가 할당되기를 기다리는 중..." 수 면 150 대 fi 완료
