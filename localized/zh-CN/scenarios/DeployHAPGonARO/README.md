...
接下来，请检查先决条件。 本部分会检查以下先决条件：RedHat OpenShift 和 kubectl。 
...
在本部分中，你将为 Azure Red Hat OpenShift (ARO) 群集创建服务主体。 SP_NAME 变量将保留服务主体的名称。 SUBSCRIPTION_ID 变量将存储你的 Azure 订阅 ID，可以使用 az account show 命令对该变量进行检索。 此 ID 是将角色分配给服务主体所必需的。 在此之后，使用 az ad sp create-for-rbac 命令创建服务主体。 最后，从 servicePrincipalInfo 变量中提取服务主体的 ID 和机密，并将其输出为 JSON 对象。 稍后将使用这些值通过 Azure 对 ARO 群集进行身份验证。
...
在本部分中，你将部署 Azure Red Hat OpenShift (ARO) 群集。 ARO_CLUSTER_NAME 变量将保留 ARO 群集的名称。 az aro create 命令将使用指定的名称、资源组、虚拟网络、子网和服务主体部署 ARO 群集。 完成此过程可能需要 30 分钟。
