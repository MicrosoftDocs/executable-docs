...
若要查看 cloud-init 的运行状况，请创建一个 VM，在上面安装 LEMP 堆栈并运行一个受 SSL 证书保护的简单 WordPress 应用。 以下 cloud-init 配置会安装所需的程序包、创建 Wordpress 网站，然后初始化并启动该网站。
...
通过向应用程序 URL 应用 curl 方法验证应用程序是否正在运行：
