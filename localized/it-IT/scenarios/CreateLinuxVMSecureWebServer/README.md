...
# Creare un server Web NGINX protetto tramite HTTPS
...
> * Configurare e proteggere la rete di Azure
...
Elenco di tutte le variabili di ambiente necessarie per eseguire questa esercitazione:
...
Prima di creare una macchina virtuale Linux sicura, creare un gruppo di risorse con az group create. Nell'esempio seguente viene creato un gruppo di risorse uguale al contenuto della variabile MY_RESOURCE_GROUP_NAME* nella posizione specificata dal contenuto *della variabile *REGION*:
...
## Configurare la rete VM

Usare az network vnet create per creare una rete virtuale denominata *$MY_VNET_NAME* con una subnet denominata *$MY_SN_NAME*nel *gruppo di risorse $MY_RESOURCE_GROUP_NAME*.
...
Usare az network public-ip create per creare un indirizzo IPv4 pubblico con ridondanza della zona standard denominato *$MY_PUBLIC_IP_NAME* in *$MY_RESOURCE_GROUP_NAME*.
...
Le regole di sicurezza dei gruppi di sicurezza di rete consentono di filtrare il tipo di traffico di rete consentito in ingresso e in uscita dalle subnet della rete virtuale e dalle interfacce di rete. Per altre informazioni sui gruppi di sicurezza di rete, vedere [Panoramica dei gruppi di sicurezza di rete](https://learn.microsoft.com/azure/virtual-network/network-security-groups-overview).
...
Aprire le porte 22 (SSH), 80 (HTTP) e 443 (HTTPS) per consentire il traffico SSH e Web
...
Infine, creare la scheda di interfaccia di rete :And finally create the Network Interface Card (NIC):
...
## Generare un certificato e archiviarlo in Azure Key Vault

Azure Key Vault consente di proteggere chiavi crittografiche e segreti, come certificati e password. Key Vault semplifica il processo di gestione dei certificati e consente di mantenere il controllo delle chiavi che accedono a tali certificati. È possibile creare un certificato autofirmato in Key Vault o caricarne uno esistente, un certificato attendibile di cui si è già proprietari. Per questa esercitazione si creeranno certificati autofirmato all'interno dell'insieme di credenziali delle chiavi e successivamente si inseriranno questi certificati in una macchina virtuale in esecuzione. Questo processo assicura che, durante la distribuzione, in un server Web vengano installati i certificati più aggiornati.

L'esempio seguente crea un insieme di credenziali delle chiavi di Azure denominato *$MY_KEY_VAULT* nell'area *scelta $REGION* con un criterio di conservazione di 7 giorni. Ciò significa che una volta eliminato un segreto, una chiave, un certificato o un insieme di credenziali delle chiavi, rimarrà recuperabile per un periodo configurabile da 7 a 90 giorni di calendario.
...
## Creare un certificato e un archivio in Azure Key Vault

A questo punto verrà generato un certificato autofirmato con az keyvault certificate create che usa i criteri di certificato predefiniti:
...
Infine, è necessario preparare il certificato in modo che possa essere usato durante il processo di creazione della macchina virtuale. A tale scopo, è necessario ottenere l'ID del certificato con az keyvault secret list-versions e convertire il certificato con az vm secret format. L'esempio seguente assegna l'output di questi comandi a delle variabili per semplificarne l'uso nei passaggi successivi:
...
Creare quindi una macchina virtuale con il comando az vm create. Usare il parametro --custom-data per passare il file di configurazione cloud-init denominato *cloud-init-nginx.txt*.
Cloud-init è un approccio diffuso per personalizzare una macchina virtuale Linux al primo avvio. Cloud-init consente di installare pacchetti e scrivere file o configurare utenti e impostazioni di sicurezza. Quando cloud-init viene eseguito durante il processo di avvio iniziale, non sono previsti passaggi aggiuntivi o agenti necessari per applicare la configurazione.
Quando si crea una macchina virtuale, certificati e chiavi vengono archiviati nella directory /var/lib/waagent/ protetta. In questo esempio viene installato e configurato il server Web NGINX.
...
L'esempio seguente crea una macchina virtuale denominata *myVMName$UNIQUE_POSTFIX*:
...
## Distribuzione dell'estensione AKV per vm $vm_name per recuperare il certificato $cert_name da AKV $akv_name..."
...
## Esplorare il sito Web sicuro

Verificare che l'applicazione sia in esecuzione visitando l'URL dell'applicazione:
