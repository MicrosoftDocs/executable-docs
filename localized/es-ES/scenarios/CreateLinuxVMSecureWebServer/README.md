...
# Crear un servidor web NGINX protegido a través de HTTPS
...
> * Configurar y proteger Redes de Azure
...
Si decide instalar y usar la CLI localmente, en este tutorial es preciso que ejecute la versión 2.0.30 u otra posterior. Ejecute `az --version` para encontrar la versión. Si necesita instalarla o actualizarla, vea [Instalación de la CLI de Azure]( https://learn.microsoft.com//cli/azure/install-azure-cli ).

## Declaración de variables

Lista de todas las variables de entorno que necesitará para ejecutar este tutorial:
...
Antes de poder crear una máquina virtual Linux segura, cree un grupo de recursos con az group create. En el ejemplo siguiente se crea un grupo de recursos igual al contenido de la variable *MY_RESOURCE_GROUP_NAME* en la ubicación especificada por el contenido de la variable *REGION*:
...
## Configurar la red de máquina virtual

Use az network vnet create para crear una red virtual denominada *$MY_VNET_NAME* con una subred denominada *$MY_SN_NAME* en el grupo de recursos *$MY_RESOURCE_GROUP_NAME*.
...
Use az network public-ip create para crear una dirección IPv4 pública estándar con redundancia de zona llamada *$MY_PUBLIC_IP_NAME* en *$MY_RESOURCE_GROUP_NAME*.
...
Puertos abiertos 22 (SSH), 80 (HTTP) y 443 (HTTPS) para permitir el tráfico SSH y web
...
Y, por último, cree la tarjeta de interfaz de red (NIC):
...
## Genere un certificado y almacénelo en Azure Key Vault

Azure Key Vault protege claves y secretos criptográficos, como certificados y contraseñas. Key Vault ayuda a agilizar el proceso de administración de certificados y le permite mantener el control de las claves que acceden a ellos. Puede crear un certificado autofirmado en Key Vault o cargar un certificado de confianza existente que ya posea. En este tutorial, crearemos certificados autofirmados dentro de Key Vault y, después, insertaremos estos certificados en una máquina virtual en ejecución. Este proceso garantiza que los certificados más actualizados se instalan en un servidor web durante la implementación.

En el ejemplo siguiente se crea una instancia de Azure Key Vault denominada *$MY_KEY_VAULT* en la región elegida *$REGION* con una directiva de retención de 7 días. Esto significa que una vez que se elimina un secreto, una clave, un certificado o un almacén de claves, no se podrá recuperar durante un período configurable de 7 a 90 días naturales.
...
## Crear un certificado y almacén en Azure Key Vault

Ahora vamos a generar un certificado autofirmado con az keyvault certificate create que use la directiva de certificado predeterminada:
...
Por último, es necesario preparar el certificado para que se pueda usar durante el proceso de creación de la máquina virtual. Para ello, es necesario obtener el id. del certificado con az keyvault secret list-versions y convertir el certificado con az vm secret format. En el ejemplo siguiente se asigna la salida de estos comandos a las variables para facilitar su uso en los pasos siguientes:
...
Ahora cree una máquina virtual con el comando az vm create. Use el parámetro --custom-data para pasar el archivo de configuración cloud-init, denominado *cloud-init-nginx.txt*.
...
Cuando crea una máquina virtual, los certificados y las claves se almacenan en el directorio /var/lib/waagent/ protegido. En este ejemplo, estamos instalando y configurando el servidor web NGINX.
...
En el ejemplo siguiente se crea una máquina virtual denominada *myVMName$UNIQUE_POSTFIX*:
...
## Implementar la extensión de AKV para la VM $vm_name para recuperar el certificado $cert_name de AKV $akv_name..."
...
## Examinar su sitio web seguro

Compruebe que la aplicación se está ejecutando visitando la dirección URL de la aplicación:
