...
A continuación, compruebe si hay requisitos previos. En esta sección se comprueban los siguientes requisitos previos: RedHat OpenShift y kubectl. 
...
## Creación de una entidad de servicio para el clúster de ARO

En esta sección, creará una entidad de servicio para el clúster de Red Hat OpenShift (ARO) en Azure. La variable SP_NAME contendrá el nombre de la entidad de servicio. La variable SUBSCRIPTION_ID, que se puede recuperar mediante el comando az account show, almacenará el identificador de suscripción de Azure. Este identificador es necesario para asignar roles a la entidad de servicio. Después, cree la entidad de servicio mediante el comando az ad sp create-for-rbac. Por último, extraiga el identificador y el secreto de la entidad de servicio de la variable servicePrincipalInfo, como un objeto JSON. Estos valores se usarán más adelante para autenticar el clúster de ARO con Azure.
