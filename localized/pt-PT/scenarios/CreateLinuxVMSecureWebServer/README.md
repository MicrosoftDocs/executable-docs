---
title: Crie um servidor Web NGINX protegido via HTTPS
...
# Crie um servidor Web NGINX protegido via HTTPS
...
Se você optar por instalar e usar a CLI localmente, este tutorial exigirá que você esteja executando a CLI do Azure versão 2.0.30 ou posterior. Executar `az --version` para localizar a versão. Se precisar de instalar ou atualizar, veja [Install Azure CLI (Instalar o Azure CLI)]( https://learn.microsoft.com//cli/azure/install-azure-cli ).

## Declaração de Variável

Lista de todas as variáveis de ambiente necessárias para executar este tutorial:
...
Antes de criar uma VM Linux segura, crie um grupo de recursos com az group create. O exemplo a seguir cria um grupo de recursos igual ao conteúdo da variável MY_RESOURCE_GROUP_NAME* no local especificado pela variável *contents *REGION:*
...
Use az network vnet create para criar uma rede virtual chamada $MY_VNET_NAME com uma sub-rede chamada *$MY_SN_NAME no *grupo de recursos $MY_RESOURCE_GROUP_NAME***.*
...
Use az network public-ip create para criar um endereço IPv4 público com redundância de zona padrão chamado *$MY_PUBLIC_IP_NAME no *$MY_RESOURCE_GROUP_NAME*.*
...
Abra as portas 22 (SSH), 80 (HTTP) e 443 (HTTPS) para permitir o tráfego SSH e da Web
...
## Gerar um certificado e armazená-lo no Cofre da Chave do Azure

O Azure Key Vault salvaguarda as chaves criptográficas e os segredos, como os certificados ou as palavras-passe. O Key Vault ajuda a simplificar o processo de gestão de chaves e permite-lhe manter o controlo das chaves que acedem a esses certificados. Pode criar um certificado autoassinado no Key Vault ou carregar um certificado fidedigno que já possui. Para este tutorial, criaremos certificados autoassinados dentro do Cofre da Chave e, posteriormente, injetaremos esses certificados em uma VM em execução. Este processo garante que são instalados os certificados mais atualizados num servidor Web durante a implementação.

O exemplo a seguir cria um Cofre de Chaves do Azure chamado *$MY_KEY_VAULT* na região *escolhida $REGION* com uma política de retenção de 7 dias. Isso significa que, uma vez que um segredo, chave, certificado ou cofre de chaves seja excluído, ele permanecerá recuperável por um período configurável de 7 a 90 dias corridos.
...
## Criar um certificado e armazenar no Cofre de chaves do Azure

Agora vamos gerar um certificado autoassinado com az keyvault certificate create que usa a política de certificado padrão:
...
Finalmente, precisamos preparar o certificado para que ele possa ser usado durante o processo de criação da VM. Para fazer isso, precisamos obter o ID do certificado com az keyvault secret list-versions, e converter o certificado com az vm secret format. O exemplo seguinte atribui o resultado destes comandos a variáveis para uma utilização mais fácil nos passos que se seguem:
...
Quando criar uma VM, os certificados e as chaves são armazenados no diretório /var/lib/waagent/ protegido. Neste exemplo, estamos instalando e configurando o servidor web NGINX.
...
O exemplo a seguir implanta uma VM e instala a extensão para habilitar o logon do Azure AD para uma VM Linux. As extensões de VM são pequenos aplicativos que fornecem tarefas de configuração e automação pós-implantação em máquinas virtuais do Azure.
