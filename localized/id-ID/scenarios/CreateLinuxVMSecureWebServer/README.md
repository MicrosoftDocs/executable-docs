---
title: Membuat Server Web NGINX Diamankan melalui HTTPS
description: Tutorial ini menunjukkan cara membuat Server Web NGINX Diamankan melalui HTTPS.
author: mbifeld
ms.author: mbifeld
ms.topic: article
ms.date: 11/28/2023
...
# Membuat Server Web NGINX Diamankan melalui HTTPS
...
> * Menyiapkan dan mengamankan Azure Networking
...
Jika Anda memilih untuk menginstal dan menggunakan CLI secara lokal, tutorial ini mengharuskan Anda menjalankan Azure CLI versi 2.0.30 atau yang lebih baru. Jalankan `az --version` untuk menemukan versinya. Jika Anda perlu memasang atau meningkatkan, lihat [Memasang Azure CLI]( https://learn.microsoft.com//cli/azure/install-azure-cli ).

## Deklarasi Variabel

Daftar semua variabel lingkungan yang anda perlukan untuk menjalankan tutorial ini:
...
Sebelum Anda dapat membuat VM Linux yang aman, buat grup sumber daya dengan az group create. Contoh berikut membuat grup sumber daya yang sama dengan konten variabel *MY_RESOURCE_GROUP_NAME* di lokasi yang ditentukan oleh REGION* konten *variabel:
...
## Menyiapkan Jaringan VM

Gunakan az network vnet create untuk membuat jaringan virtual bernama *$MY_VNET_NAME* dengan subnet bernama *$MY_SN_NAME*di *grup sumber daya $MY_RESOURCE_GROUP_NAME*.
...
Gunakan az network public-ip create untuk membuat alamat IPv4 publik zona redundan standar bernama *$MY_PUBLIC_IP_NAME* di *$MY_RESOURCE_GROUP_NAME*.
...
Buka port 22 (SSH), 80 (HTTP) dan 443 (HTTPS) untuk memungkinkan lalu lintas SSH dan Web
...
Dan akhirnya membuat Kartu Antarmuka Jaringan (NIC):
...
## Membuat sertifikat dan menyimpannya di Azure Key Vault

Azure Key Vault melindungi kunci dan rahasia kriptografi, seperti sertifikat atau kata sandi. Key Vault membantu mempermudah proses manajemen sertifikat dan memungkinkan Anda mempertahankan kontrol kunci yang mengakses sertifikat tersebut. Anda dapat membuat sertifikat yang ditandatangani sendiri di dalam Key Vault, atau mengunggah sertifikat tepercaya yang ada yang Anda miliki. Untuk tutorial ini, kita akan membuat sertifikat yang ditandatangani sendiri di dalam Key Vault dan setelah itu menyuntikkan sertifikat ini ke VM yang sedang berjalan. Proses ini memastikan bahwa sertifikat terbaru diinstal di server web selama penyebaran.

Contoh berikut membuat Azure Key Vault bernama *$MY_KEY_VAULT* di wilayah *yang dipilih $REGION* dengan kebijakan penyimpanan 7 hari. Ini berarti setelah rahasia, kunci, sertifikat, atau brankas kunci dihapus, itu akan tetap dapat dipulihkan selama periode yang dapat dikonfigurasi 7 hingga 90 hari kalender.
...
## Membuat sertifikat dan menyimpan di Azure key Vault

Sekarang mari kita buat sertifikat yang ditandatangani sendiri dengan az keyvault certificate create yang menggunakan kebijakan sertifikat default:
...
Terakhir, kita perlu menyiapkan sertifikat sehingga dapat digunakan selama proses pembuatan VM. Untuk melakukannya, kita perlu mendapatkan ID sertifikat dengan az keyvault secret list-versions, dan mengonversi sertifikat dengan format rahasia az vm. Contoh berikut menetapkan output perintah ini ke variabel untuk kemudahan penggunaan di langkah-langkah berikutnya:
...
Sekarang buat VM dengan az vm create. Gunakan parameter --custom-data untuk meneruskan file konfigurasi cloud-init, bernama *cloud-init-nginx.txt*.
Cloud-init adalah pendekatan yang digunakan secara luas untuk mengustomisasi Linux VM saat boot pertama kali. Anda dapat menggunakan cloud-init untuk memasang paket dan menulis file, atau untuk mengonfigurasi pengguna dan keamanan. Saat cloud-init berjalan selama proses boot awal, tidak ada langkah tambahan atau agen yang diperlukan untuk menerapkan konfigurasi Anda.
Saat Anda membuat VM, sertifikat dan kunci disimpan dalam direktori /var/lib/waagent/ yang dilindungi. Dalam contoh ini, kami menginstal dan mengonfigurasi server web NGINX.
...
Contoh berikut membuat VM bernama *myVMName$UNIQUE_POSTFIX*:
...
## Menyebarkan ekstensi AKV untuk VM $vm_name untuk mengambil cert $cert_name dari AKV $akv_name..."
...
Contoh berikut menyebarkan mesin virtual dan kemudian memasang ekstensi untuk mengaktifkan masuk ke Azure AD untuk mesin virtual Linux. Ekstensi VM Azure adalah aplikasi kecil yang menyediakan konfigurasi pasca-penyebaran dan tugas otomatisasi pada komputer virtual Azure.
...
## Telusuri situs web aman Anda

Validasi bahwa aplikasi berjalan dengan mengunjungi url aplikasi:
