...
Kubernetes 資訊清單檔案會定義叢集所需的狀態，例如要執行的容器映射。

在本快速入門中，您將使用資訊清單來建立執行 Azure 投票應用程式所需的所有物件。 此資訊清單包含兩個 Kubernetes 部署：

- 範例 Azure 投票 Python 應用程式。
- Redis 實例。
...
造訪公用 IP 或應用程式 URL 來驗證應用程式是否正在執行。 您可以執行下列命令來找到應用程式 URL：

> [!Note]
> 建立 POD 通常需要 2-3 分鐘的時間，且網站可透過 HTTP 連線
